<div class="container">
    <header class="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3 mb-4 border-bottom">
        <a href="{{siteUrl()}}" class="d-flex align-items-center col-md-3 mb-2 mb-md-0 text-dark text-decoration-none">
        ...
        <ul class="nav col-12 col-md-auto mb-2 justify-content-center mb-md-0">
            <li>
                <a href="{{siteUrl()}}" class="nav-link px-2 link-secondary">@trans(Anasayfa)</a>
            </li>
            <li>
                <a href="{{siteUrl('about')}}" class="nav-link px-2 link-dark">@trans(Hakkımızda)</a>
            </li>
            <li>
                <a href="{{siteUrl('contact')}}" class="nav-link px-2 link-dark">@trans(İletişim)</a>
            </li>
            <li>
                <a href="{{siteUrl('blog')}}" class="nav-link px-2 link-dark">@trans(Blog)</a>
            </li>
        </ul>
        <div class="col-md-3 text-end">
            @if(auth()->segment->get('user_name'))
                <a href="{{siteUrl('auth/profile')}}" class="btn btn-outline-primary me-2">[{{auth()->segment->get('user_name')}}]</a>
            @else
                <a href="{{siteUrl('auth/login')}}" class="btn btn-outline-primary me-2">@trans(Giriş Yap)</a>
                <a href="{{siteUrl('auth/register')}}" class="btn btn-primary">@trans(Kayıt Ol)</a>
            @endif
        </div>
    </header>
</div>